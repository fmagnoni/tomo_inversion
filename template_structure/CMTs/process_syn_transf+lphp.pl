...
#cambio path
$saclst="saclst";
#provo con path iasp91, ma credo che qualunque cosa metto vada tanto non lo usa
#$phtimes="/opt/iasp91/phtimes";
#corretto (quindi phtimes.sh deve essere nella dir in uso)
$phtimes="./phtimes.csh";
#non serve nemmeno che questo file sia nella dir tanto non lo usa
...
foreach $file (@ARGV) {
  ...
  (undef,$net,$sta,undef,$comp)=split(/\./,$filename);
  print "sta ",$sta," ", $net, " ",$comp,"\n";
  ...
  if ($opt_a) { # add station information
    ...
    print "sta ",$sta," ", $net,`egrep '$sta +$net' $opt_a`,"\n";
  ...
  #3/2/2011 Commento riga seguenta data la modifica di Qinya Jan 2011; la sposto dopo
  ...
  if ($opt_l) { #cut record...18/1/11 magnoni inserico comando sac per fare padding di zeri
     print SAC "setbb begin $lmin \n"; # magnoni 18/1/11: modifico così prende start cut che voglio
     print SAC "setbb end   $lmax \n"; # magnoni 18/1/11: modifico così prende start end che voglio
     print SAC "cuterr fillz \n";  #magnoni 18/1/11
     print SAC "cut %begin% %end% \n";  #magnoni 18/1/11 potevo anche mettere cut $lmin $lmax
     print SAC "r $outfile\n";
     print SAC "cut off\n";}
  ...
  if ($opt_t)  { # filter record
    print "Filtering records... lp hp \n";
    print SAC "rtrend\n rmean\n taper\n";
    if ($opt_f) {
	  #7/2/14 provo processing diverso	
      # printf SAC ("trans from none to none freq %12.6f %12.6f %12.6f %12.6f\n",$f0,$f1,$f2,$f3);}
	  # 7/2/2014 magnoni: per copiare processamento di Laura-Elisa ma essere anche coerente con proc dato, prima faccio un transfer largo
      printf SAC ("trans from none to none freq 0.001 0.005 0.5 1\n");
      #printf SAC ("bp n $poles p $pass co %10.5f %10.5f\n",$f1,$f2);}
      printf SAC ("lp n %4d p $pass cor %12.6f\n",$poles,$f2);
      printf SAC ("hp n %4d p $pass cor %12.6f\n",$poles,$f1);}
  ...
  #spostata qui data modifica Qinya Jan 2011
  if ($opt_s) {print SAC "interp delta $dt\n";}
