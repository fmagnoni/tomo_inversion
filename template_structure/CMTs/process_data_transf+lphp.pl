...
#$saclst="/Applications/sac/bin/saclst";
#provo con path di iasp91
#$phtimes="/opt/iasp91/phtimes";
#quello corretto è (e phtimes.csh va nelle cartella in uso)
$phtimes="./phtimes.csh";
...
#3/2/2011 in accordo con modifica di Qinya sposto questo dopo "transfer instrument response"
  ...
  if ($opt_i or $opt_R) {  # transfer instrument response
    ...
    if ($opt_i) { # assume pz file in the same dir
	  ...
	  # @respfiles=glob("$opt_R/${network}.${sta}.${khole}.${comp}*.RESP");   # 15/6/2012 magnoni
      @respfiles=glob("$opt_R/RESP.${network}.${sta}..${comp}");  # 15/6/2012 magnoni #25.10.2014 casarotti (- khole)
	...
	# 26/11/2012 magnoni: con to none porta i velocimetri in displacement quindi metto to vel
	# printf SAC ("trans from evalresp fname $respfile to vel freq %12.6f%12.6f%12.6f%12.6f \n",
	# 	  $f0,$f1,$f2,$f3);
	# 7/2/2014 magnoni: per copiare processamento di Laura-Elisa, prima faccio un transfer largo
    
    printf SAC ("trans from evalresp fname $respfile to vel freq 0.001 0.005 0.5 1\n");
	printf SAC "mul 1e-9\n";
    # 7/2/2014 magnoni: aggiungo bp per copiare processamento di Laura-Elisa
    #printf SAC ("bp n %4d p $pass cor %12.6f %12.6f\n",$poles,$f1,$f2);}}
    print "    filter with lp hp\n";
